    ...
    return (
        <Link href={destination} className="border-2 border-slate-300 p-5 block bg-slate-100 hover:bg-slate-200">
            <div className="flex w-full">
                <Image src={image} height="200" width="300" style={{ objectFit: 'cover', height:'200px', width:'300px' }}  alt="" />
            </div>
            <div className="mt-3 text-lg font-bold">{title}</div>
            <div>AUD ${numeral(price).format("0,0")}</div>
            <div className="flex justify-between txt-sm mt-3">
                <div>
